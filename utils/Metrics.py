...
def DiceS(output, target, epsilon=1e-2):
...
def dice_score(output, target, num_classes):
    mean_dice = 0
    for i in range(0,num_classes,1):
        binary_out = torch.zeros_like(output.cpu())
        binary_out[output.cpu()==i] = 1
        binary_y = torch.zeros_like(target.squeeze(1))
        binary_y[target==i] = 1
        element_dice = DiceS(binary_out, binary_y)
        mean_dice += element_dice
    return mean_dice/num_classes
    #element_prec = Precision(binary_out, binary_y)
    #element_recall = Recall(binary_out, binary_y)
...
class SegmentationMetrics(object):
    r"""Calculate common metrics in semantic segmentation to evaluate model preformance.
    ...
    def __init__(self, eps=1e-5, average=True, ignore_label=True, activation='0-1'):
        self.eps = eps
    ...
    def _calculate_multi_metrics(self, gt, pred, class_num):
        ...
        # Manual addition by aashay: Ignore 1st label also if OOD present in labels.
        # matrix = matrix[:, 1:]
